...
def plot_dim_reduction_improved(data, mapper_dict, default_features=None, default_hue_info=None, 
                       row_width=950, row_height=500, plotly_marker_size=1.5, bokeh_marker_size=3, return_results=False):
    '''
    Функция принимает на вход данные и набор 2D/3D dimension-редукторов через mapper_dict.
    Отрисовывает эмбеддинги этих данных в наиболее удобных форматах: 3D - plotly, 2D - bokeh с CDS sharing'ом
    ...
    n_bokeh = len(bokeh_figs)
    if n_bokeh > 0:
        plot_width = round(row_width / (3 + 0.1))

        # Рассчитываем количество строк в сетке
        rows = (n_bokeh // 3) + (1 if n_bokeh % 3 > 0 else 0)

        # Создаем сетку Bokeh
        grid = gridplot([bokeh_figs[i:i + 3] for i in range(0, n_bokeh, 3)], width=plot_width, height=row_height)
        show(grid)
    ...
    if n_plotly > 0:
        ...
        # Рассчитываем количество строк в сетке
        rows = (n_plotly // 3) + (1 if n_plotly % 3 > 0 else 0)

        # Создаем сетку Plotly
        plotly_widgets = []
        for i in range(0, n_plotly, 3):
            figs_subset = plotly_figs[i:i + 3]
            layout = figs_subset[0].layout
            layout.update({'width': plot_width, 'height': row_height,
                            'title_x': 0.5, 'title_font_size': 13, 'legend_itemsizing': 'constant'})
            for fig in figs_subset:
                fig.update_layout(legend={})
                fig.update_traces(marker_size=plotly_marker_size)
            new_fig = go.FigureWidget(figs_subset[0].data, layout=layout)
            new_fig.update_layout(legend={})
            plotly_widgets.append(new_fig)
